...
- `GET /dashboard` or `GET /api/dashboard`
  - Requires active session
  - Returns user, session id, request count, and serving node
- `GET /session` or `GET /api/session`
...
Backend includes lightweight quality scripts and smoke tests:

- `cd backend && npm run lint` (syntax check for backend entrypoints)
- `cd backend && npm run build` (syntax check used as a build sanity gate)
- `cd backend && npm test` (Node test runner for smoke tests in `backend/tests`)

### Production security hardening
- Backend startup fails fast in production if `SESSION_SECRET` is missing/empty.
- `SESSION_SECRET` must be at least 32 characters and not a weak default.
- `COOKIE_SECURE=true` is required when `NODE_ENV=production`.
