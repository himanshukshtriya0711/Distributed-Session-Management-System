...
function ensureSecureProductionConfig() {
  const isProduction = process.env.NODE_ENV === "production";
  const weakSecrets = new Set(["change-me-in-production", "changeme", "secret", "password"]);
  ...
  if (!process.env.SESSION_SECRET || !process.env.SESSION_SECRET.trim()) {
    throw new Error("SESSION_SECRET must be explicitly set in production and cannot be empty");
  }
  ...
  if (weakSecrets.has(sessionSecret.toLowerCase())) {
    throw new Error("SESSION_SECRET value is too weak for production");
  }
